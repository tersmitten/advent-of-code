...
foreach ($lines as $i => $line) {
	if (preg_match('/^(\d+) (\d+) (\d+) (\d+)$/', $line, $matches) > 0) {
		$instructions[] = array_map('intval', array_slice($matches, 1, 4));
	}
}
...
$possibleOpcodeMapping = $opcodeMapping = [];
...
foreach ($samples as $i => $sample) {
	...
	$possibleOpcodeMapping[$opcode] = [];

	foreach ($opcodes as $opcodeName) {
		$actual = $opcodeName($sample['before'], $a, $b, $c);
		$same = $actual == $expected;
		if ($same && !in_array($opcodeName, $opcodeMapping, true)) {
			$possibleOpcodeMapping[$opcode][] = $opcodeName;
		}
	}

	if (count($possibleOpcodeMapping[$opcode]) === 1) {
		$opcodeMapping[$opcode] = $possibleOpcodeMapping[$opcode][0];
	}
}

// print_r(compact('opcodeMapping'));

foreach ($instructions as $instruction) {
	[$opcode, $a, $b, $c] = $instruction;
	$opcodeName = $opcodeMapping[$opcode];
	$registers = $opcodeName($registers, $a, $b, $c);
}

// print_r(compact('registers'));
